...
class _MemoryLanePageState extends ConsumerState<MemoryLanePage>
  ...
  Widget _buildTimelineTab(BuildContext context) {
    ...
    // Engagement stats
    final masteredCount = assignments.where((a) => a.visitedCount == a.assignedCount && a.assignedCount > 0).length;
    final totalXp = assignments.fold<int>(0, (sum, a) => sum + (a.visitedCount * 50));
    final totalLocations = assignments.fold<int>(0, (sum, a) => sum + a.assignedCount);
    final visitedLocations = assignments.fold<int>(0, (sum, a) => sum + a.visitedCount);
    final overallProgress = totalLocations > 0 ? visitedLocations / totalLocations : 0.0;
      ...
      child: RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            if (index == 0) {
              return _buildQuestSummaryHeader(
            ...
            return QuestCard(assignment: assignment);
          },
        ),
      ),
    );
  ...
  Widget _buildQuestSummaryHeader({
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildStatItem('Quests', '$totalQuests', Icons.map),
              _buildStatDivider(),
              _buildStatItem('Mastered', '$masteredCount', Icons.emoji_events),
              _buildStatDivider(),
              _buildStatItem('Progress', '${(progress * 100).toInt()}%', Icons.auto_graph),
            ],
          ),
