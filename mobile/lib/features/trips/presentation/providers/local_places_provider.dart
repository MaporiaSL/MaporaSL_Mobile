...
final localPlacesProvider = FutureProvider<List<Place>>((ref) async {
  try {
    final String response = await rootBundle.loadString(
      'assets/data/curated_places.json',
    );
    ...
    for (var item in data) {
      try {
        ...
        // Map District
        if (normalized.containsKey('district')) {
          normalized['districtId'] = normalized['district'];
        }
...
final localPlacesWithDistanceProvider = FutureProvider<List<PlaceWithDistance>>((
  ref,
) async {
  ...
  final userLocAsync = ref.watch(
    userLocationProvider.select((v) => v),
  ); // Watch just the value if possible
